...
diamond_size = max_sales * 0.03 if max_sales > 0 else 1
...
ax.tick_params(colors='white')
ax.set_ylim(0, max_sales * 1.2 or 100)
ax.grid(True, linestyle='--', alpha=0.3, color='#2b3f63')
for spine in ax.spines.values():
    spine.set_color('#2b3f63')
...
# 在每个柱子上方显示销量 (蓝色单元格样式)
max_sales = max(sales_data, default=0)
for i, (xi, val) in enumerate(zip(x, sales_data)):
    if val > 0:
        # 蓝色背景单元格
        box_width = 0.5
        box_height = max_sales * 0.08 if max_sales > 0 else 10
        box = FancyBboxPatch((xi - box_width/2, val + max_sales*0.02), 
                             box_width, box_height,
                             boxstyle='round,pad=0.02,rounding_size=0.1',
                             facecolor='#3498db', edgecolor='white', linewidth=1)
        ax.add_patch(box)
        ax.text(xi, val + max_sales*0.02 + box_height/2, f'{int(val)}',
...
# 添加蓝色销量标签
max_sales = max(sales_2025, default=0)
for xi, val in zip(x, sales_2025):
    if val > 0:
        box_h = max_sales * 0.06 if max_sales > 0 else 10
        box_w = 0.5
        box = FancyBboxPatch((xi - box_w/2, val + max_sales*0.02), 
                             box_w, box_h,
                             boxstyle='round,pad=0.01,rounding_size=0.08',
                             facecolor='#3498db', edgecolor='white', linewidth=0.5)
        ax.add_patch(box)
        ax.text(xi, val + max_sales*0.02 + box_h/2, f'{int(val):,}',
...
ax.tick_params(colors='white')
ax.set_ylim(0, max_sales * 1.35 if max_sales > 0 else 100)
ax.grid(True, axis='y', linestyle='--', alpha=0.3, color='#2b3f63')
for spine in ax.spines.values():
    spine.set_color('#2b3f63')
